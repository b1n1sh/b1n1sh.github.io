...
(function() {
  ...
  WebPreferences.prototype.initDefaults_ = function() {
    ...
    this.litepageSubresourceRedirectOrigin = null;
  };
  ...
  WebPreferences.validate = function(messageValidator, offset) {
    ...
    // validate WebPreferences.litepageSubresourceRedirectOrigin
    err = messageValidator.validateStructPointer(offset + codec.kStructHeaderSize + 280, origin$.Origin, false);
    if (err !== validator.validationError.NONE)
        return err;
  ...
  WebPreferences.encodedSize = codec.kStructHeaderSize + 288;
  ...
  WebPreferences.decode = function(decoder) {
    ...
    val.canvas2dLayersEnabled = (packed >> 3) & 1 ? true : false;
    val.antialiased2dCanvasDisabled = (packed >> 4) & 1 ? true : false;
    val.antialiasedClips2dCanvasEnabled = (packed >> 5) & 1 ? true : false;
    val.acceleratedFiltersEnabled = (packed >> 6) & 1 ? true : false;
    val.deferredFiltersEnabled = (packed >> 7) & 1 ? true : false;
    packed = decoder.readUint8();
    val.containerCullingEnabled = (packed >> 0) & 1 ? true : false;
    val.allowRunningInsecureContent = (packed >> 1) & 1 ? true : false;
    val.disableReadingFromCanvas = (packed >> 2) & 1 ? true : false;
    val.strictMixedContentChecking = (packed >> 3) & 1 ? true : false;
    val.strictPowerfulFeatureRestrictions = (packed >> 4) & 1 ? true : false;
    val.allowGeolocationOnInsecureOrigins = (packed >> 5) & 1 ? true : false;
    val.strictlyBlockBlockableMixedContent = (packed >> 6) & 1 ? true : false;
    val.blockMixedPluginContent = (packed >> 7) & 1 ? true : false;
    packed = decoder.readUint8();
    val.passwordEchoEnabled = (packed >> 0) & 1 ? true : false;
    val.shouldClearDocumentBackground = (packed >> 1) & 1 ? true : false;
    val.enableScrollAnimator = (packed >> 2) & 1 ? true : false;
    val.threadedScrollingEnabled = (packed >> 3) & 1 ? true : false;
    val.prefersReducedMotion = (packed >> 4) & 1 ? true : false;
    val.touchEventFeatureDetectionEnabled = (packed >> 5) & 1 ? true : false;
    val.dontSendKeyEventsToJavascript = (packed >> 6) & 1 ? true : false;
    val.barrelButtonForDragEnabled = (packed >> 7) & 1 ? true : false;
    packed = decoder.readUint8();
    val.syncXhrInDocumentsEnabled = (packed >> 0) & 1 ? true : false;
    val.targetBlankImpliesNoOpenerEnabledWillBeRemoved = (packed >> 1) & 1 ? true : false;
    val.allowNonEmptyNavigatorPlugins = (packed >> 2) & 1 ? true : false;
    val.supportsMultipleWindows = (packed >> 3) & 1 ? true : false;
    val.viewportEnabled = (packed >> 4) & 1 ? true : false;
    val.viewportMetaEnabled = (packed >> 5) & 1 ? true : false;
    val.shrinksViewportContentsToFit = (packed >> 6) & 1 ? true : false;
    val.smoothScrollForFindEnabled = (packed >> 7) & 1 ? true : false;
    ...
    packed = decoder.readUint8();
    val.mainFrameResizesAreOrientationChanges = (packed >> 0) & 1 ? true : false;
    val.initializeAtMinimumPageScale = (packed >> 1) & 1 ? true : false;
    val.smartInsertDeleteEnabled = (packed >> 2) & 1 ? true : false;
    val.spatialNavigationEnabled = (packed >> 3) & 1 ? true : false;
    val.navigateOnDragDrop = (packed >> 4) & 1 ? true : false;
    val.fakeNoAllocDirectCallForTestingEnabled = (packed >> 5) & 1 ? true : false;
    val.recordWholeDocument = (packed >> 6) & 1 ? true : false;
    val.cookieEnabled = (packed >> 7) & 1 ? true : false;
    packed = decoder.readUint8();
    val.acceleratedVideoDecodeEnabled = (packed >> 0) & 1 ? true : false;
    val.userGestureRequiredForPresentation = (packed >> 1) & 1 ? true : false;
    val.textTracksEnabled = (packed >> 2) & 1 ? true : false;
    val.immersiveModeEnabled = (packed >> 3) & 1 ? true : false;
    val.doubleTapToZoomEnabled = (packed >> 4) & 1 ? true : false;
    val.fullscreenSupported = (packed >> 5) & 1 ? true : false;
    val.textAutosizingEnabled = (packed >> 6) & 1 ? true : false;
    val.forceDarkModeEnabled = (packed >> 7) & 1 ? true : false;
    packed = decoder.readUint8();
    val.hideDownloadUi = (packed >> 0) & 1 ? true : false;
    val.presentationReceiver = (packed >> 1) & 1 ? true : false;
    val.mediaControlsEnabled = (packed >> 2) & 1 ? true : false;
    val.doNotUpdateSelectionOnMutatingSelectionRange = (packed >> 3) & 1 ? true : false;
    val.pictureInPictureEnabled = (packed >> 4) & 1 ? true : false;
    val.translateServiceAvailable = (packed >> 5) & 1 ? true : false;
    val.lazyLoadEnabled = (packed >> 6) & 1 ? true : false;
    val.allowMixedContentUpgrades = (packed >> 7) & 1 ? true : false;
    packed = decoder.readUint8();
    val.alwaysShowFocus = (packed >> 0) & 1 ? true : false;
    val.touchDragDropEnabled = (packed >> 1) & 1 ? true : false;
    ...
    val.lazyImageFirstKFullyLoad =
        decoder.decodeMapPointer(new codec.Enum(EffectiveConnectionType), codec.Int32);
    val.litepageSubresourceRedirectOrigin =
  ...
  WebPreferences.encode = function(encoder, val) {
    ...
    packed |= (val.canvas2dLayersEnabled & 1) << 3
    packed |= (val.antialiased2dCanvasDisabled & 1) << 4
    packed |= (val.antialiasedClips2dCanvasEnabled & 1) << 5
    packed |= (val.acceleratedFiltersEnabled & 1) << 6
    packed |= (val.deferredFiltersEnabled & 1) << 7
    encoder.writeUint8(packed);
    packed = 0;
    packed |= (val.containerCullingEnabled & 1) << 0
    packed |= (val.allowRunningInsecureContent & 1) << 1
    packed |= (val.disableReadingFromCanvas & 1) << 2
    packed |= (val.strictMixedContentChecking & 1) << 3
    packed |= (val.strictPowerfulFeatureRestrictions & 1) << 4
    packed |= (val.allowGeolocationOnInsecureOrigins & 1) << 5
    packed |= (val.strictlyBlockBlockableMixedContent & 1) << 6
    packed |= (val.blockMixedPluginContent & 1) << 7
    encoder.writeUint8(packed);
    packed = 0;
    packed |= (val.passwordEchoEnabled & 1) << 0
    packed |= (val.shouldClearDocumentBackground & 1) << 1
    packed |= (val.enableScrollAnimator & 1) << 2
    packed |= (val.threadedScrollingEnabled & 1) << 3
    packed |= (val.prefersReducedMotion & 1) << 4
    packed |= (val.touchEventFeatureDetectionEnabled & 1) << 5
    packed |= (val.dontSendKeyEventsToJavascript & 1) << 6
    packed |= (val.barrelButtonForDragEnabled & 1) << 7
    encoder.writeUint8(packed);
    packed = 0;
    packed |= (val.syncXhrInDocumentsEnabled & 1) << 0
    packed |= (val.targetBlankImpliesNoOpenerEnabledWillBeRemoved & 1) << 1
    packed |= (val.allowNonEmptyNavigatorPlugins & 1) << 2
    packed |= (val.supportsMultipleWindows & 1) << 3
    packed |= (val.viewportEnabled & 1) << 4
    packed |= (val.viewportMetaEnabled & 1) << 5
    packed |= (val.shrinksViewportContentsToFit & 1) << 6
    packed |= (val.smoothScrollForFindEnabled & 1) << 7
    encoder.writeUint8(packed);
    ...
    packed = 0;
    packed |= (val.mainFrameResizesAreOrientationChanges & 1) << 0
    packed |= (val.initializeAtMinimumPageScale & 1) << 1
    packed |= (val.smartInsertDeleteEnabled & 1) << 2
    packed |= (val.spatialNavigationEnabled & 1) << 3
    packed |= (val.navigateOnDragDrop & 1) << 4
    packed |= (val.fakeNoAllocDirectCallForTestingEnabled & 1) << 5
    packed |= (val.recordWholeDocument & 1) << 6
    packed |= (val.cookieEnabled & 1) << 7
    encoder.writeUint8(packed);
    packed = 0;
    packed |= (val.acceleratedVideoDecodeEnabled & 1) << 0
    packed |= (val.userGestureRequiredForPresentation & 1) << 1
    packed |= (val.textTracksEnabled & 1) << 2
    packed |= (val.immersiveModeEnabled & 1) << 3
    packed |= (val.doubleTapToZoomEnabled & 1) << 4
    packed |= (val.fullscreenSupported & 1) << 5
    packed |= (val.textAutosizingEnabled & 1) << 6
    packed |= (val.forceDarkModeEnabled & 1) << 7
    encoder.writeUint8(packed);
    packed = 0;
    packed |= (val.hideDownloadUi & 1) << 0
    packed |= (val.presentationReceiver & 1) << 1
    packed |= (val.mediaControlsEnabled & 1) << 2
    packed |= (val.doNotUpdateSelectionOnMutatingSelectionRange & 1) << 3
    packed |= (val.pictureInPictureEnabled & 1) << 4
    packed |= (val.translateServiceAvailable & 1) << 5
    packed |= (val.lazyLoadEnabled & 1) << 6
    packed |= (val.allowMixedContentUpgrades & 1) << 7
    encoder.writeUint8(packed);
    packed = 0;
    packed |= (val.alwaysShowFocus & 1) << 0
    packed |= (val.touchDragDropEnabled & 1) << 1
    ...
    encoder.encodeStructPointer(origin$.Origin, val.litepageSubresourceRedirectOrigin);
  };
